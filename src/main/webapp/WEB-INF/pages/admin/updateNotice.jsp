                    ...
                    <form class="layui-form">
                        <div class="layui-row layui-form-item">
                            ...
                            <input type="text" lay-verify="required" placeholder="请输入标题" autocomplete="off"
                                   class="layui-input" style="margin-top: 10px" id="title" value="${notice.title}">
                        </div>

                        <div class="layui-row layui-form-item">
                            <label>内容：</label><br>
                            <div id="editor" style="margin-top: 10px">${notice.content}</div>
                        </div>
                                    ...
                                    <table class="layui-table">
                                        ...
                                        <c:forEach items="${notice.noticeFiles}" var="file" varStatus="s">
                                            <tr id="tr${file.id}">
                                                <td>${file.filename}</td>
                                                <td>未知</td>
                                                <td><span style="color: #5FB878;">原上传文件</span></td>
                                                <td><a class="layui-btn layui-btn-mini layui-btn-danger"
                                                       style="margin-left: 10px" onclick="delfun(${file.id});">删除</a>
                                                </td>
                                            </tr>
                                        </c:forEach>
                                        <script>
                                            function delfun(id) {
                                                ...
                                                layer.confirm('此操作会删除公告中文件，确定删除吗？', {icon: 3, title:'提示'}, function(index){
                                                    $.post("${pageContext.request.contextPath}/noticeFile/delNoticeFile", {
                                                        "id": id
                                                    });
...
<script>
    layui.config({
        ...
        $("#submit").click(function () {
            ...
            $.post("${pageContext.request.contextPath}/notice/updateNotice", {
                "id":${notice.id},
                "title": $("#title").val(),
